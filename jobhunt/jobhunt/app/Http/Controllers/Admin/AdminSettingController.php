...
class AdminSettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if($request->hasFile('logo')) {
            ...
            unlink(public_path('uploads/settings/'.$obj->logo));
            $ext = $request->file('logo')->extension();
            $final_name = 'logo'.'.'.$ext;
            $request->file('logo')->move(public_path('uploads/settings/'),$final_name);
            $obj->logo = $final_name;
        }
        ...
        if($request->hasFile('favicon')) {
            ...
            unlink(public_path('uploads/settings/'.$obj->favicon));
            $ext = $request->file('favicon')->extension();
            $final_name = 'favicon'.'.'.$ext;
            $request->file('favicon')->move(public_path('uploads/settings/'),$final_name);
