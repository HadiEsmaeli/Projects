...
$get = $conn->prepare("SELECT * FROM `props` JOIN `request` ON `props`.id = `request`.propid WHERE `request`.userid = :req");
$get->execute([':req'=>$_SESSION['userid']]);
...
<div class="container">
    ...
    <div class="row mb-5">
        <?php if( $get->rowCount() > 0 ) : ?>
            ...
            <?php endforeach; ?>
        <?php else : ?>
            <div class="bg-success text-white">you did not request property yet.</div>            
        <?php endif; ?>
    </div>        
</div>
